...
class Tracker {
  ...
  report(event, properties = {}) {
    ...
    if (typeof wx !== 'undefined' && wx.request) {
      ...
      console.warn('Tracker: 不在微信小程序环境中，事件已缓存');
      this.log(log, properties);
    }
  }
  ...
  log(event, properties = {}) {
    ...
    if (typeof wx !== 'undefined' && wx.getStorage) {
      ...
      if (this.memoryQueue.length >= MAX_CACHE_SIZE) {
        this.sendBatch(this.memoryQueue);
        this.memoryQueue = [];
      }
      
      this.memoryQueue.push(log);
      console.log('Event added to memory queue, total:', this.memoryQueue.length);
    }
  ...
  async sendBatch(logs) {
    ...
    try {
      if (typeof wx !== 'undefined' && wx.request) {
        ...
        // 非微信小程序环境，可以发送到其他接口或控制台输出
        console.log('Tracker batch (non-wx environment):', logs);
        // 这里可以添加其他环境的发送逻辑
      }
    } catch (err) {
  ...
  async flush() {
    if (typeof wx !== 'undefined' && wx.getStorage) {
      ...
      if (this.memoryQueue && this.memoryQueue.length > 0) {
        const queueCopy = [...this.memoryQueue];
        this.memoryQueue = [];
        this.sendBatch(queueCopy);
      }
    }
  }
...
if (typeof module !== 'undefined' && module.exports) {
  ...
  // 浏览器环境
  window.Tracker = trackerInstance;
} else if (typeof global !== 'undefined') {
  // Node.js 环境
  global.Tracker = trackerInstance;
}
